    ...
    try {
      ...
      if (faceWidth < 50 || faceHeight < 50) {
        ...
        return new Response(
          JSON.stringify({
            error:
              "Je gezicht is te klein in de foto. Upload een close-up selfie waarbij je gezicht goed zichtbaar is.",
          }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
      ...
      // Store face analysis
      const { error: analysisError } = await supabase.from("face_analysis").upsert({
        user_id: user.id,
        photo_url: photoUrl,
      ...
      // CRITICAL FIX: Update profile with ALL required fields INCLUDING photo_url
      ...
      const { error: profileError } = await supabase.from("profiles").upsert({
        ...
        date_of_birth: dateOfBirth,
        photo_url: photoUrl, // ✅ CRITICAL: Save photo_url so profile is marked complete
      });
      ...
      if (photoPath) {
        try {
          const { error: deleteError } = await supabase.storage.from("profile-photos-temp").remove([photoPath]);
